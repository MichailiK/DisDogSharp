...
using DisDogSharp.Common.Utilities;
using DisDogSharp.Entities;
using DisDogSharp.Enums;
using DisDogSharp.EventArgs;
using DisDogSharp.Net;
using DisDogSharp.Net.Udp;
using DisDogSharp.Net.WebSocket;
using DisDogSharp.VoiceNext.Codec;
using DisDogSharp.VoiceNext.Entities;
using DisDogSharp.VoiceNext.EventArgs;
...
namespace DisDogSharp.VoiceNext;
...
public sealed class VoiceNextConnection : IDisposable
{
	...
	internal VoiceNextConnection(DiscordClient client, DiscordGuild guild, DiscordChannel channel, VoiceNextConfiguration config, VoiceServerUpdatePayload server, VoiceStateUpdatePayload state)
	{
		...
		this._userSpeaking = new("VNEXT_USER_SPEAKING", TimeSpan.Zero, this._discord.EventErrorHandler);
		this._userJoined = new("VNEXT_USER_JOINED", TimeSpan.Zero, this._discord.EventErrorHandler);
		this._userLeft = new("VNEXT_USER_LEFT", TimeSpan.Zero, this._discord.EventErrorHandler);
		this._voiceReceived = new("VNEXT_VOICE_RECEIVED", TimeSpan.Zero, this._discord.EventErrorHandler);
		this._voiceSocketError = new("VNEXT_WS_ERROR", TimeSpan.Zero, this._discord.EventErrorHandler);
